	...
	private boolean registerJar(URLClassLoader classLoader, URL jar)
		...
		try(JarFile jarFile = connection.getJarFile()) {
			...
			while(jarEntries.hasMoreElements()) {
				...
				if(entry.endsWith("/InteropRegisterer.class") || entry.endsWith("$InteropRegisterer.class") || entry
					.equals("InteropRegisterer.class")) {
					...
					try {
						...
						if(registerObject instanceof InteropRegisterer) {
							...
							throw new IncompatibleJarException(jar,
								new ClassCastException("Unknown type for InteropRegisterer"));
						}
					} catch(InstantiationException | IllegalAccessException | ClassNotFoundException | RuntimeException | Error | NoSuchMethodException | InvocationTargetException e) {
