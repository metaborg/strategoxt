 ...
 * Time
 */
ATerm STRSGLR_settimer()
{
  struct rusage rusage;
  getrusage(RUSAGE_SELF,&rusage);
  timer = rusage.ru_utime.tv_sec * 1000000L + rusage.ru_utime.tv_usec;
  return (ATerm) ATempty;
}

ATerm STRSGLR_gettimer()
{
  struct rusage rusage;  
  long current;
  getrusage(RUSAGE_SELF,&rusage);
  current = rusage.ru_utime.tv_sec * 1000000L + rusage.ru_utime.tv_usec;
  return (ATerm) ATmakeInt(current - timer);
}
