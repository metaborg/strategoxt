...
/**
 * Builds an application term using the given term factory. The constructor name is expected as a
 * term argument, the argument list is expected as the current term.
 * 
 * @author seba
 */
public class UnsafeBuild extends AbstractPrimitive {
	...
	if (factory == null) {
		...
		factory = ((TypesmartTermFactory) smartFactory).getWrappedFactory();
	    } else {
	...
	IStrategoTerm unsafeTerm = factory.makeAppl(factory.makeConstructor(ctr, argArray.length),
		argArray);
