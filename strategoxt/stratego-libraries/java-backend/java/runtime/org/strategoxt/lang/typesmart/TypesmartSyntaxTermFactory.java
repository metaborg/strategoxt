...
public class TypesmartSyntaxTermFactory extends TypesmartTermFactory {
	...
	private class Key {
		...
		public boolean equals(Object o) {
			return o instanceof Key && ((Key) o).ctr.equals(ctr)
	...
	/**
	 * Clone of {@link TypesmartTermFactory#registerTypesmartFactory(Context, ITermFactory)}. Needed
	...
	public static ITermFactory registerTypesmartFactory(Context context, ITermFactory factory) {
		...
		if (factory instanceof TypesmartTermFactory) {
			return new TypesmartSyntaxTermFactory(context,
					((TypesmartTermFactory) factory).getWrappedFactory());
		}
		if (factory instanceof AbstractWrappedTermFactory) {
			...
			if (oldBaseFactory instanceof TypesmartTermFactory) {
				ITermFactory newBaseFactory = new TypesmartSyntaxTermFactory(context,
						((TypesmartTermFactory) oldBaseFactory).getWrappedFactory());
				((AbstractWrappedTermFactory) factory).replaceBaseFactory(newBaseFactory);
			}
			((AbstractWrappedTermFactory) factory).replaceBaseFactory(registerTypesmartFactory(
					context, oldBaseFactory));
	...
	public static boolean isTypesmartSyntax(ITermFactory factory) {
		if (factory instanceof AbstractWrappedTermFactory) {
			return ((AbstractWrappedTermFactory) factory)
					.hasBaseFactory(TypesmartSyntaxTermFactory.class);
		}
		return false;
	}
