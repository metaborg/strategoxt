...
public class Context extends StackTracer implements IAsyncCancellable {
	...
	private final UncaughtExceptionHandler.Finalizer exceptionHandlerFinalizer = new UncaughtExceptionHandler.Finalizer(
			exceptionHandler);
	...
	protected Context(ITermFactory factory,
			Map<String, IOperatorRegistry> operatorRegistryMap,
			List<IOperatorRegistry> operatorRegistries, boolean skipCompat) {
		this.factory = factory;
	...
	/**
	 * Registers a new class loader used for dynamical class loading (using
	 * {@link SSL_EXT_java_call}).
	 * 
	 ...
	 * <code>C</code> is a class that would be dynamically loaded by a Stratego
	 * program.
	 */
	...
	public void addOperatorRegistry(IOperatorRegistry or) {
		IOperatorRegistry previous = operatorRegistryMap.put(
				or.getOperatorRegistryName(), or);
		if (previous == null) {
	...
	public IStrategoTerm invokeStrategyCLI(Strategy strategy, String appName,
			String... args) throws StrategoExit, StrategoException {

		IStrategoList input = toCLITerm(appName, args);

		// Launch with a clean operand stack when launched from SSL_java_call,
		// Ant, etc.
		if (new Exception().getStackTrace().length > 20) {
			return new StackSaver(strategy).invokeStackFriendly(this, input,
					NO_STRATEGIES, NO_TERMS);
		} else {
			return strategy.invoke(this, input);
		}
	}

	public IStrategoTerm invokeStrategyCLI(String strategy, String appName,
			String... args) throws MissingStrategyException, StrategoErrorExit,
	...
	public IStrategoTerm invokeStrategy(String strategy, IStrategoTerm input)
			throws MissingStrategyException, StrategoErrorExit, StrategoExit,
			StrategoException {
	...
	public final IStrategoTerm invokePrimitive(String name, IStrategoTerm term,
	...
	public IStrategoTerm invokePrimitive(AbstractPrimitive primitive,
			IStrategoTerm term, Strategy[] sargs, IStrategoTerm[] targs) {
		if (primitive == null)
		...
		try {
			if (primitive.call(interopContext,
					InteropCallT.toInteropCallTs(sargs, this), targs)) {
			...
			throw new StrategoException("Exception in execution of primitive '"
					+ primitive.getName() + "'", e);
