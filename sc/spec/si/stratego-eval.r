...
// Strategy definition and call
  ...
  EvalCall(eval) :
    ...
    where <table-lookup> ("senv", (f,<length>as)) => SDef(f, xs, s);
          <zip(\ (x, a) -> 
                 <table-push> ("senv", (x,0), SDef(x,[],a)) \ )> (xs, as);
          <eval> App(s, t) => t';
          <map(\ x -> <table-pop>("senv", (x,0))\ )> xs
  ...
  EvalLet(eval) :
    ...
    where <map(\sdef@SDef(f,as,_) -> 
                <table-push>("senv", (f,<length>as), sdef)\ )> sdefs;
          <eval> App(s, t) => t';
          <map(\ SDef(f,as,_) -> <table-pop>("senv", (f,<length>as))\ )> sdefs
...
rules
  ...
  EvalScope(eval) :
    App(Scope(xs, s), t) -> t'
    where <map(\ x -> <table-push>("tenv", Var(x), NULL())\ )> xs;
	  <eval> App(s, t) => t';
          <map(\ x -> <table-pop>("tenv", Var(x))\ )> xs
...
rules
  ...
  SubsVar :
    x@Var(_) -> t
    where <table-lookup>("tenv", x) => t; not(?NULL())
